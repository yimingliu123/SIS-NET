...
import numpy as np
from models.deeplab_resnet import semantic
...
class FPNHead(nn.Module):    ################  lym
    def __init__(self, num_in=256, num_mid=128, num_out=128):
        super().__init__()

        self.block0 = nn.Conv2d(num_in, num_mid, kernel_size=3, padding=1, bias=False)
        self.block1 = nn.Conv2d(num_mid, num_out, kernel_size=3, padding=1, bias=False)
    def forward(self, x):
        #print("head qian" , x.size())
        x = nn.functional.relu(self.block0(x), inplace=True)
        x = nn.functional.relu(self.block1(x), inplace=True)
        #print("head hou" , x.size())
        return x
...
class PoolNet(nn.Module):
    ...
    def forward(self, x):
        x_size = x.size()
        lateral , infos = self.base(x)
        #
        # print("lateral0", lateral[0].size())
        # print("lateral1", lateral[1].size())
        # print("lateral2", lateral[2].size())
        # print("lateral3", lateral[3].size())
        # print("lateral4", lateral[4].size())
        #
        # print("infos0", infos[0].size())
        # print("infos1", infos[1].size())
        # print("infos2", infos[2].size())

        merge0 =  self.td1(lateral[1]+infos[0])
        merge1 =  self.td1(lateral[2]+infos[1]+ F.interpolate(merge0, lateral[2].size()[2:], mode='bilinear', align_corners=True))
        merge2 =  self.td1(lateral[3] + infos[2] + F.interpolate(merge1, lateral[3].size()[2:], mode='bilinear', align_corners=True))


        map0 = F.interpolate(self.head1(lateral[0]), lateral[3].size()[2:], mode='bilinear', align_corners=True)
        map1 = F.interpolate(self.head1(merge0), lateral[3].size()[2:], mode='bilinear', align_corners=True)
        map2 = F.interpolate(self.head2(merge1), lateral[3].size()[2:], mode='bilinear', align_corners=True)
        map3 = F.interpolate(self.head3(merge2), lateral[3].size()[2:], mode='bilinear', align_corners=True)


        # print("map0 size ", map0.size())
        # print("map1 size ", map1.size())
        # print("map2 size ", map2.size())
        # print("map3 size ", map3.size())

        smoothed = self.smooth(torch.cat([map3, map2, map1, map0], dim=1))
        # print("cat size ", smoothed.size())

        smoothed = F.interpolate(smoothed, lateral[4].size()[2:], mode='bilinear', align_corners=True)
        # print("cat.interpolate size ", smoothed.size())

        smoothed = self.smooth2(smoothed + lateral[4])
        # print("merge 3 size ", smoothed.size())
        smoothed = F.interpolate(smoothed, x_size[2:], mode='bilinear', align_corners=True)
...
def get_poolnet():
    return PoolNet(semantic())

def get_nets(model_config):
    return get_poolnet(), get_discriminator(model_config)
